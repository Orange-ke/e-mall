  ...
  <div class="container">
    ...
    <div class="header">
      <van-row type="flex" style="box-sizing: border-box; background: #ffd31c">
        <van-col span="20">
          <van-search placeholder="搜索商品名称" v-model="searchValue" shape="round" @search="search" background="transparent"></van-search>
        </van-col>
      ...
      <van-tabs v-model="tabActive" sticky @click="onTabClick(tabActive)" color="#ffd31c" class="popupContainer">
        <van-tab
...
export default {
  ...
  mounted: function () {
    let vm = this
    axios
      .get('/api/Address/tp5ProductAddressList?cateId=' + vm.$route.query.sid)
      .then(res => {
        console.log(res)
